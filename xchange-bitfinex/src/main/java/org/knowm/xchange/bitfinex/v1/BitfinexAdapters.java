...
public final class BitfinexAdapters {
    ...
    public static ExchangeMetaData adaptMetaData(
        ...
        Iterator it = withdrawFees.entrySet().iterator();
        while (it.hasNext()) {
            Map.Entry<Currency, BigDecimal> pair = (Map.Entry) it.next();
            if (currencies.get(pair.getKey()) == null) {
                CurrencyMetaData currencyMetaData = new CurrencyMetaData(0, pair.getValue());
                currencies.put(pair.getKey(), currencyMetaData);
            } else {
                final CurrencyMetaData oldMetaData = currencies.get(pair.getKey());
                CurrencyMetaData newMetaData =
                        new CurrencyMetaData(oldMetaData.getScale(), pair.getValue());
                currencies.put(pair.getKey(), newMetaData);
            }
        }
        return metaData;
    }
